...
```mermaid
graph TD
    User([User Topic Input]) --> Orch[🧠 Curiosity Orchestrator]
    
    subgraph Multi-Agent Intelligence Pipeline
        Orch --> R[🔍 Agent 1: Research Intelligence]
        R --> A[🧠 Agent 2: Assumption Detection]
        A --> B[👁️ Agent 3: Human Blindspot Agent]
        B --> C[⚡ Agent 4: Contradiction Detection]
        C --> F[🔮 Agent 5: Future Consequence Agent]
        F --> Rev[🔄 Agent 6: Reverse Thinking Agent]
        Rev --> QG[❓ Agent 7: Question Generation Agent]
        QG --> RK[⭐ Agent 8: Question Ranking Agent]
    end
    
    subgraph Question Novelty Engine
        RK --> DNA[🧬 Question DNA Calculation]
        DNA --> Exp[🛡️ Explainable AI Discovery Path]
    end
